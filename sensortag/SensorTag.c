...
#define MAG_DEFAULT_PERIOD                    1000
...
static uint8 advertData[] =
{
  ...
  /*custom stuff by brendan
  0x03, //length of this data
  GAP_ADTYPE_SERVICE_DATA,
  0xBF, //arbitrary magic number for debugging
  0xA7, //arbitrary magic number for debugging*/
  
  0x08,
  GAP_ADTYPE_FLAGS,
  0xBB, //header for identifying
  0x00, //6 bytes for magnetometer data
  0x00,
  0x00,
  0x00,
  0x00,
...
int16 mag_cali[3];
int8 alreadyCalibrated = FALSE; //custom brendan
...
void brendan_sensors() {
    //custom brendan - start magnetomoter sensor
    magEnabled = TRUE;
    osal_set_event( sensorTag_TaskID, ST_MAGNETOMETER_SENSOR_EVT);
    
    HalAccSetRange(HAL_ACC_RANGE_4G);
    osal_set_event( sensorTag_TaskID, ST_ACCELEROMETER_SENSOR_EVT);
    accConfig = ST_CFG_SENSOR_ENABLE;
...
static void readAccData(void)
{
  ...
  if (HalAccRead(aData))
  {
    ...
    const int ACCEL_BASE = 15;
    
    advertData[ACCEL_BASE + 0] = aData[0]; //custom brendan
    advertData[ACCEL_BASE + 1] = aData[1];
    advertData[ACCEL_BASE + 2] = aData[2];
    GAPRole_SetParameter( GAPROLE_ADVERT_DATA, sizeof( advertData ), advertData );
...
int16 uints2int(uint8 lsb, uint8 msb) {
...
static void readMagData( void )
{
  ...
  if (HalMagRead(mData))
  {
    ...
    //custom brendan. add mag data to advert array   
    if (!alreadyCalibrated) {
       alreadyCalibrated = 1;
       mag_cali[0] = uints2int(mData[0], mData[1]);
       mag_cali[1] = uints2int(mData[2], mData[3]);
       mag_cali[2] = uints2int(mData[4], mData[5]);
    }
    

    int16 magData[3];
    
    magData[0] = uints2int(mData[0], mData[1]) - mag_cali[0];
    magData[1] = uints2int(mData[2], mData[3]) - mag_cali[1];
    magData[2] = uints2int(mData[4], mData[5]) - mag_cali[2];
    
    const int MAG_BASE = 6;
    
    advertData[MAG_BASE+0] = magData[0] & 0xFF;
    advertData[MAG_BASE+1] = magData[0] >> 8;
    advertData[MAG_BASE+2] = magData[1] & 0xFF;
    advertData[MAG_BASE+3] = magData[1] >> 8;
    advertData[MAG_BASE+4] = magData[2] & 0xFF;
    advertData[MAG_BASE+5] = magData[2] >> 8;
    GAPRole_SetParameter( GAPROLE_ADVERT_DATA, sizeof( advertData ), advertData );
